...
function App() {
    ...
    useEffect(() => {
        axios.get("http://127.0.0.1:8000/star-wars/event/").then((resp) => {
            let temp = [];
            resp.data.forEach((rdm) => {
                temp.push({id:rdm.id, start:rdm.start_time, end: rdm.end_time, name: rdm.event_name})
            })
            console.log(temp)
            setDatesBooked(temp)
        })
                ...
                if (
                    ...
                    setDatesBooked([
                        ...datesBooked,
                        {
                            ...
                            start: bookSchedule.start_date + "T" + bookSchedule.start_time + ":00",
                            end: bookSchedule.end_date + "T" + bookSchedule.end_time + ":00",
                        },
                    ]);
                    axios.post("http://127.0.0.1:8000/star-wars/event/",{
                        id: parseInt((new Date().valueOf()).toString().substring(0,6)),
                        event_name: bookSchedule.name,
                        start_time: bookSchedule.start_date + " " + bookSchedule.start_time + ":00",
                        end_time: bookSchedule.end_date + " " + bookSchedule.end_time + ":00",
    ...
    const deleteSession = (id) => {
        axios.delete(`http://127.0.0.1:8000/star-wars/event/${id}`).then(response => console.log(response))
    ...
    return ( 
        ...
        <div className = "selectedDate__detail" > {
            ...
            selectedDateEvents.map((sde, sdeindex) => ( 
                ...
                <p> <span> From - </span> { sde.start }  </p>  
                <p>
                <span> To - </span> { sde.end }  
                 </p> 
                 </div> 
                  <Button variant = "dark"
                onClick = {
                    () => deleteSession(sde.id)
                } >
                Delete </Button>  
                </div>
            ))
